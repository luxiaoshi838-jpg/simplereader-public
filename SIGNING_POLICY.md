...
2. `tools/verify-ui-policy.sh` 与对应单元测试必须通过后才能生成正式 APK。
3. 分页、缓存、搜索、目录和书签定位修改不得顺带修改确认 UI；确需修改时必须由项目所有者明确提出。
4. 目录/书签使用确认的 v600 侧边面板；背景使用颜色、纹理、质感三层；横向三种模式必须保持不同实现。
5. 上下模式必须是单一连续正文，不得使用按页容器、吸附翻页或页间空白。
6. 横向分页不得为每页预留空白；只有章节最后一页可因内容不足自然留白。
