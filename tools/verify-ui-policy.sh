...
grep -Fq 'navigationBarInsetPx + oneCharacterPx * 3' "$reader" || fail "reader lower limit must leave three current-font characters above navigation bar"
...
grep -q 'ExistingWorkPolicy.REPLACE' app/src/main/java/com/simplereader/app/worker/ShelfCacheWorker.kt || fail "new shelf cache action must replace and cancel the older unique task"
