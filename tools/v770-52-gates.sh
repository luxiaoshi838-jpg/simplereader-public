...
cd "$ROOT"
TMP="$(mktemp tools/v770-52-generated.XXXXXX.sh)"
LEGACY_GATE17="tools/v749-17-gates.sh"
LEGACY_GATE18="tools/v749-18-gates.sh"
LEGACY_GATE19="tools/v750-19-gates.sh"
BACKUP17="$(mktemp tools/v749-17-gates.v770-backup.XXXXXX.sh)"
BACKUP18="$(mktemp tools/v749-18-gates.v770-backup.XXXXXX.sh)"
BACKUP19="$(mktemp tools/v750-19-gates.v770-backup.XXXXXX.sh)"
...
source = source.replace('2098000757', version_code)
source = source.replace("'\"757\"'", f"'\"{version_name}\"'")
source = source.replace('v757/rule113 versions missing', f'v{version_name}/rule115 versions missing')
source = source.replace('v757 version + catalog rule113', f'v{version_name} version + catalog rule115')
...
if old_checkpoint not in source:
    raise SystemExit('V770+ gate adaptation failed: V757 checkpoint assertion not found')
...
if old_anchor not in source:
    raise SystemExit('V770+ gate adaptation failed: V757 fallback anchor assertion not found')
