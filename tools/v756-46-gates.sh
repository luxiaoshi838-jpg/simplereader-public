...
for token in ['explicitNumberedTitle','numberLeadingUnit','hasValidStructuralTail','numericOnly.matches(s)']:
    if token not in direct:
        raise SystemExit(f'rule113 direct structural-boundary guard missing: {token}')
for token in ['hasGluedPrefixedStructuralText','if (hasGluedPrefixedStructuralText(normalized)) return null','startsWithNumeralToken','if (startsWithNumeralToken(normalized)) return null']:
    if token not in parser:
        raise SystemExit(f'rule113 fallback structural-boundary guard missing: {token}')
for token in ['3节课','3节 课','3节：课','第3节课','第3节 课','12章鱼','12章 鱼','一条','1天','3 天','三 天','12 公里']:
    if token not in test:
        raise SystemExit(f'rule113 boundary regression test missing: {token!r}')
PY
pass 36 'rule113 direct+fallback boundary guards and spaced/glued regression cases present'
