...
class UiRegressionPolicyTest {
    ...
    fun `confirmed v625 single complete background model is restored`() {
        ...
        assertTrue(picker.contains("ReaderBackgrounds.Category.values()"))
        assertTrue(picker.contains("ReaderBackgrounds.options(activeCategory)"))
        assertTrue(picker.contains("ReaderBackgrounds.selection(activeCategory, option.id)"))
