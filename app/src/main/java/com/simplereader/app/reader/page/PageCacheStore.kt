...
object PageCacheStore {
    ...
    /**
     * A recognition record is reusable only when source identity AND catalog-rule identity match.
     * This restores the version-aware cache contract used by the historical reader chain.
     */
    fun isRecognitionCurrent(
        ...
        root.optString("fileName").equals(fileName, ignoreCase = true) &&
            ...
            root.optInt("catalogRuleVersion", -1) == TxtParser.CATALOG_RULE_VERSION &&
            root.optBoolean("completed", false)
    }.getOrDefault(false)

    /** True only for a visible catalog generated by the current recognition rules. */
    ...
    /** Remove every derived catalog/page artifact, including normalized TXT chapter metadata. */
    fun clearDerivedCatalogAndPages(context: Context, bookId: Long) {
        clearBook(context, bookId)
    }
