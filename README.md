...
v614 在已确认阅读实现上新增并修正：目录标题支持“连续汉字 + 括号数字”；目录规则变化不再自动使既有缓存失效；全书架缓存范围由两个明确选项决定；TXT/EPUB 使用用户提供的格式默认封面，EPUB 有真实封面时仍优先真实封面；阅读页纯色、纹理和质感改用用户提供素材；未捕获崩溃会在下次启动显示可复制日志。
