...
- 版本：v614
- versionCode：`2098000614`
- versionName：`614`
- 包名：`com.simplereader.app`
- 正式证书 SHA-256：`315d7bbf06b2a0a16ea7efd7a5c7cd8e6371ab9b0f40ae380cc416e1472c8648`

v614 在已确认阅读实现上新增并修正：目录标题支持“连续汉字 + 括号数字”；目录规则变化不再自动使既有缓存失效；全书架缓存范围由两个明确选项决定；TXT/EPUB 使用用户提供的格式默认封面，EPUB 有真实封面时仍优先真实封面；阅读页纯色、纹理和质感改用用户提供素材；未捕获崩溃会在下次启动显示可复制日志。阅读页整体边界保持 v612 原样，不移动根布局；仅将正文顶部内边距由 26dp 调为 50dp，使正文最高位置降低一个字符高度。
